const User = require("../model/userModel_dummy");
...
const createSendToken = (user, status, res) => {
    ...
    //we gona set a cookie :3
    const cookieOptions = {
        expires: new Date(Date.now() + process.env.JWT_COOKIE_EXPIRES_IN * 24 * 60 * 60 * 1000),
        httpOnly: true,
        // secure: true //this shall be uncommented while in production
    }
...
exports.protect = catchAsync(async (req, res, next) => {
    ...
    // verify the token
    //verify also ascepts a callback function, but we will make it return a promise
